  ...
  return (
    <section aria-label="筛选运行记录" className="task-filter-card rounded-2xl bg-muted/20 px-4 py-3">
      <div className="task-filter-controls grid min-w-0 gap-2 sm:grid-cols-2 xl:grid-cols-[150px_150px_minmax(220px,1fr)_220px_auto] xl:items-center">
        ...
        <label className="task-filter-pill col-start-3 row-start-1 flex h-10 cursor-pointer items-center justify-center gap-2 rounded-full border border-border/45 bg-background px-4 text-sm text-muted-foreground hover:bg-muted has-[:checked]:bg-destructive/10 has-[:checked]:text-destructive">
          <input
            type="checkbox"
            className="sr-only"
            checked={onlyAnomaly}
