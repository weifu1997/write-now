...
export const DEFAULT_ANTI_AI_RULES: DefaultAntiAiRuleDefinition[] = [
  {
    ...
    rewriteSuggestion: "把心理解释改成动作、语气、停顿、对白、决策变化或直接删掉；同一篇里换用不同方向，不要全部改成小动作。",
  ...
  },
  {
    ...
    rewriteSuggestion: "删除升华句，回到具体动作、现场或悬而未决的处境；多数时候直接删掉即可，不必补一句替代。",
  ...
  },
  {
    ...
    rewriteSuggestion: "压缩或合并说明段，把关键信息拆进对话、动作和决策里；能推断出来的说明直接删掉。",
  ...
  {
    key: "risk-faint-quantity-modifiers",
    name: "滥用「一丝、一抹」式模糊修饰",
    type: "risk",
    severity: "medium",
    description: "正文频繁出现「一丝、一抹、几分」这类模糊量词，情绪和氛围显得含糊，缺少具体画面。",
    detectPatterns: ["一丝", "一抹", "一缕", "几分", "些许"],
    rewriteSuggestion: "多数情况下直接删掉模糊量词，让具体动作和细节自己传递感受；确实需要保留时，换成看得见、听得着的具体细节，不要换成另一个模糊量词。",
    promptInstruction: "慎用「一丝、一抹、几分」等模糊量词修饰情绪和氛围；能删就删，需要时用具体可感的细节代替。",
    autoRewrite: false,
    enabled: true,
    globalBaselineEnabled: true,
  },
  {
    key: "risk-reflexive-reaction-adverbs",
    name: "滥用「不禁、不由得」式反应",
    type: "risk",
    severity: "medium",
    description: "情绪被写成条件反射，「不禁、不由得、下意识地」一带而过，人物反应失去重量和选择感。",
    detectPatterns: ["不禁", "不由得", "不由自主", "下意识地", "忍不住"],
    rewriteSuggestion: "删掉反应副词，直接写出人物做了什么、说了什么；也可以让反应带一点犹豫、克制或错位，让人物的选择感回来。",
    promptInstruction: "避免用「不禁、不由得、下意识地」带出情绪反应；直接写人物的动作、语气和决定，让反应有具体过程。",
    autoRewrite: false,
    enabled: true,
    globalBaselineEnabled: true,
  },
  {
    key: "risk-frozen-air-atmosphere-cliche",
    name: "滥用「空气仿佛凝固」式氛围套话",
    type: "risk",
    severity: "medium",
    description: "用「空气仿佛凝固、时间静止、落针可闻」渲染紧张，出现多了紧张感反而变得廉价。",
    detectPatterns: ["空气仿佛凝固", "空气凝固", "空气骤然", "时间仿佛静止", "时间仿佛停滞", "落针可闻", "死一般的寂静"],
    rewriteSuggestion: "用现场里具体的声音、半截对话、人物当场做出的决定或突然的停顿呈现紧张；也可以直接删掉这句，让对话和事件本身制造张力，不要惯性地补一个小动作。",
    promptInstruction: "不要用「空气仿佛凝固、时间静止」这类套话渲染气氛；用现场具体的声音、停顿和人物反应来呈现紧张。",
    autoRewrite: false,
    enabled: true,
    globalBaselineEnabled: true,
  },
  {
    key: "risk-uniform-expression-formula",
    name: "滥用「嘴角勾起」式表情公式",
    type: "risk",
    severity: "medium",
    description: "「嘴角勾起一抹弧度、眸光一暗」这类表情公式在所有角色脸上轮播，人物表情失去个性。",
    detectPatterns: ["嘴角勾起", "唇角勾起", "勾起一抹", "嘴角微微上扬", "嘴角噙着", "眸光一暗", "眸色一沉"],
    rewriteSuggestion: "换成这个角色独有的表情习惯或说话方式，或者从对手的反应来写他的表情；不同角色不要共用同一套表情公式。",
    promptInstruction: "少用「嘴角勾起、眸光一暗」这类通用表情公式；每个角色的情绪表达要贴合其性格和当时的处境。",
    autoRewrite: false,
    enabled: true,
    globalBaselineEnabled: true,
  },
  {
    key: "risk-eye-flash-emotion-projection",
    name: "滥用「眼底闪过」式情绪投射",
    type: "risk",
    severity: "medium",
    description: "「眼底闪过一丝、眼中闪过一抹」用眼睛转播情绪，绕开了真正该写的情绪过程。",
    detectPatterns: ["眼底闪过", "眼中闪过", "眸中闪过", "眼中掠过", "眼底浮现", "眼底掠过"],
    rewriteSuggestion: "让情绪落在可观察的变化上——语气变了、动作停了、决定改了；也可以删掉这句，用前后行为的反差让读者自己感到波动。",
    promptInstruction: "避免「眼底闪过、眼中闪过」式情绪转播；情绪要透过行为、语气和决策的变化让读者看见。",
    autoRewrite: false,
    enabled: true,
    globalBaselineEnabled: true,
  },
  {
    ...
    type: "encourage",
    severity: "low",
    description: "鼓励加入真实但不推动主线的细节增加人味；细节方向要多样，不要只堆小动作。",
    detectPatterns: [],
    rewriteSuggestion: "按场景轮换细节方向：小动作、口头禅、对话里的绕弯、无关的环境信息、多余的顾虑都可以；避免反复使用挠头、点烟、指节抵桌面这类万能小动作。",
    promptInstruction: "适当加入真实但不推动主线的细节增加生活感；在动作、对话、语气和环境细节之间换着用，不要每次都落到同一种小动作上。",
    autoRewrite: false,
    enabled: true,
  ...
  },
  {
    ...
    detectPatterns: [],
    rewriteSuggestion: "补出人物预期与实际结果之间的差距；可以用结果偏差、对话错位、他人反应呈现，不必每次都写愣住或僵住。",
