...
export const chapterWriterPrompt: PromptAsset<ChapterWriterPromptInput, string, string> = {
  ...
  contextRequirements: [
    ...
    { group: "style_anchor_passages", priority: 60 },
  ],
  management: {
    productPrompt: true,
  ...
  render: (input, context) => {
    ...
    const coreConstraints = mode === "condense"
      ...
      : [
          ...
          "1c. 推进章必须有可见人物对白与当场互动（质问、谈判、威胁、误判、安抚或冲突），禁止整章只写流程旁白、审计说明、术语展览或单向调度。",
          "1d. 若 recent_chapters 已连续使用同一玩法引擎（例如连续查账收网、连续封门夺印），本章必须换玩法：关系博弈、公开对质、资源兑换、误伤代价、身份暴露或敌方反制等，不得再复读同构流程。",
    ...
    return [
      new SystemMessage([
        ...
        "如果存在范文锚点（style_anchor_passages）：以范文的句式节奏、细节密度与对话留白方式为风格参照进行模仿；范文只锚定写法，禁止照抄其中的具体情节、人物、地名、组织名等任何实体；范文与本章任务冲突时，以本章任务为准。",
        "",
        "【禁止事项】",
        "禁止引入未铺垫的重大转折。",
        "禁止跳跃式推进导致逻辑断裂。",
        ...
        "* 想写纯审计/流程旁白 -> 改为角色当场对质、讨价还价、误判或承担代价。",
        "* 想用术语堆叠制造专业感 -> 改为读者能看见的动作结果与关系压力。",
        "",
        "【输出前自查】",
        "在生成正文前，先内部确认以下三点：",
        "(1) 结尾是否形成了新的悬念或钩子？",
