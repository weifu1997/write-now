...
连载书到达 `endingRequiredBy`（用户设定的目标章数）时，最后一卷 / 最后一拍 / 目标章进入**目标跨度可见小结局**：本阶段高潮和兑现必须落地，可以留余味，但不得再开必须续写的新主线。判定使用全书绝对章序，与紧凑全书终章共用同一套累加规则。连载不套用紧凑全书完本审校，收束缺口最多记为章节级质量债，不得因此暂停自动导演。
