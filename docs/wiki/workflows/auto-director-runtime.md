...
连载书到达 `endingRequiredBy`（用户设定的目标章数）时，最后一卷 / 最后一拍 / 目标章进入**目标跨度可见小结局**：本阶段高潮和兑现必须落地，可以留余味，但不得再开必须续写的新主线。判定使用全书绝对章序，与紧凑全书终章共用同一套累加规则。目标章前约 8 章开始终局铺垫：收束主线、集齐高潮条件；最后约 3 章进入终局落地。连载不套用紧凑全书完本审校，收束缺口最多记为章节级质量债，不得因此暂停自动导演。

全书自动执行范围必须停在 `maxChapterCount`。库里如果已经多拆出标题，这些章节可以保留，但不得继续进入自动执行剩余队列。完本配置只能来自任务完本合同或小说目标章数，禁止用当前已有章节总数反推，否则超标拆章会把目标一起带跑。
