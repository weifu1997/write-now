...
滚动生产期的可信尺度由 `resolveVolumePlannedChapterBudget` 统一给出：先取 `max(加权分摊, 全书均分)` 作为下限，再与全书剩余章数取小。全书均分（`全书预算 / 卷数`，与 `buildEvenChapterBudgets` 的 base 口径一致）正是节奏板生成在无完整章节分布时的兜底尺度，校验与生成因此自洽；加权分摊保证已完成卷的口径不被抬高；剩余章数上限保证空的后续卷不会按均分扩成完整一卷。

示例：

- 150 章、4 卷、收官卷只写了 4 章：均分 37，剩余 37，可信尺度仍是 37，收官卷不会被压成 5 章。
- 213 章、前 4 卷已规划 210 章、第 5 卷为空：均分 42，剩余 3，可信尺度是 3，不得按 42 章生成新终局卷。
...
- 滚动补路线、节奏板和拆章还必须受 `completionProfile.maxChapterCount` 封顶；超过上限的拍不再生成。
...
- 若只有下限没有上限：收官卷拍满后仍会新建后续卷，空卷按全书均分生成完整节奏板，目标 213 章可能被拆到 226 甚至约 252 章，并在已写终局后再打一轮终局。
