...
自动导演确认新书方向后，正文前同步等待的资产限定为：精简故事基础、开篇世界切片、核心角色、3～5 章路线和第 1 章执行合同。完整世界、非开篇角色资料、心智快照、远期卷规划和第 2 章以后的完整合同都不是首章前置条件。开篇约束只压缩解释性上下文，不删除角色硬事实、章节任务和世界禁区。
...
- 简易书架「一键自动修复全部质量项」走 `chapterScope=quality_debt`。该入口下 `pause` / `needs_manual_review` 仍表示本章未通过，但不能跳过自动修复。同一次任务里每章最多修 2 次：第一次按验收结构化字段选择轻修或改写；轻修补丁无法安全应用，或轻修后复审仍不过，立即在本章改写，不要求用户再点一次按钮。修完本章后，若流水线闸门仍未通过，必须再走一次与章节页「重新审校」相同的独立审校来刷新 `qualityLoop`：审校得到 `recommendedAction=continue` 即关闭待跟进项，不再要求用户逐章点审校。独立审校失败时保留当前正文和待跟进项，不得中断剩余章节。`plan_misalignment` 仍不改写本章。普通写作入口保持原语义：每章最多修 1 次，验收暂停就跳过自动修复，轻量补丁失败不得升级成整章重写，也不自动套用这条独立审校收口。
