...
- 简易书架「一键自动修复全部质量项」走 `chapterScope=quality_debt`。该入口下 `pause` / `needs_manual_review` 仍表示本章未通过，但不能跳过自动修复。同一次任务里每章最多修 2 次：第一次按验收结构化字段选择轻修或改写；轻修补丁无法安全应用，或轻修后复审仍不过，立即在本章改写，不要求用户再点一次按钮。`plan_misalignment` 仍不改写本章。普通写作入口保持原语义：每章最多修 1 次，验收暂停就跳过自动修复，轻量补丁失败不得升级成整章重写。
