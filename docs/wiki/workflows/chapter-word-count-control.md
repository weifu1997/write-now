...
- 低于 softMin：沿用既有续写兜底。
- 若平台体验合同给出 softMin / softMax / hardMax，字数预算优先用平台合同，而不是一律按目标 × 0.85 / 1.15 / 1.25。番茄免费长篇硬上限是 2500，不是碰巧等于 2000 × 1.25。
- 写作图区间守卫是有限步：先压缩，低于下限再续写一次；若续写后又超过硬上限，再压缩一次。禁止循环，禁止拆章。
- 章节没有目标字数时，用书级默认章长或平台推荐章长生成预算，不再跳过长度守卫。
