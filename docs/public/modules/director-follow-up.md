...
侧栏角标只统计现在还需要处理的项：校验阻塞、失败或人工恢复、等待确认和重规划。已取消、已替代、自动通过的历史记录可以留在列表里查看，但不会把角标一直标红。

对已经不用处理的历史项，在导演跟进详情选择「收起这条记录」。收起后这条提醒不再出现在跟进列表和角标里，不会删除小说，也不会改已保存章节。正在运行、等待确认或需要人工恢复的任务不能收起。运行记录仍然只用于查看，不在那里取消或收起。
