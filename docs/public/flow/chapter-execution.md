...
质量债务应该被记录、展示和后续跟进，而不是默认阻断全局自动导演。只有明确重规划、不可恢复生成失败、数据风险或安全风险才应停止全局链。简易书架可以对全部待跟进质量项启动自动修复：按章节顺序审校并修复一章，完成后再进入下一章。一次点击会在同一章里先局部修补，仍不过或补丁打不上时自动改写，修完后再按当前正文重新审校；审校通过即关闭待跟进项。这不会重写没有质量债的章节，也不会打断全书继续写作。自动修复开始后，待审核状态提案和已经超过兑现窗口的全书伏笔不能单独暂停这一批任务。
