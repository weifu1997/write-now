...
- 选番茄免费、起点或晋江后，新书会按该平台的阅读习惯给出默认章长、开篇进入压力的速度，以及前几章该兑现到哪。
- 番茄免费长篇默认约 2000 字一章，开篇更快进入冲突；起点和晋江仍约 2800 字。你手填过的章长会保留，只提示和平台建议的差异。
- 前几章更强调当场动作和选择，减少说明书式开场；单章过长会先压缩再补写，不会自动拆成两章。
- 番茄书如果勾选无限流、诸天万界、黑科技这类和平台不合的标签，保存时会拦住并给出可改方向；明确保留才允许继续。
...
### 2026-09-08（章节范文收录、一键质量修复与全书自动门控）
...
### 2026-09-07（逾期伏笔与章节完成状态分离）

- 已超过兑现窗口的伏笔会继续保留在全书承诺记录中，但不会再把后文章节误显示为待优化或留存风险。
- 章节完成状态只根据本章正文问题判断；窗口内仍应推进的伏笔义务会继续参与本章检查。
- 自动创作遇到逾期伏笔时可以继续写作，不会仅因这类全书风险暂停或要求重规划。

### 2026-09-06（产品更名 Write Now 与导演进度提示优化）

- 产品正式更名为 Write Now，应用界面、官网介绍站、桌面应用与项目文档统一使用新名称。
- 项目仓库迁移至 [weifu1997/write-now](https://github.com/weifu1997/write-now)，README、应用内项目链接与桌面端更新地址同步指向新仓库。
- 数据库、向量知识库与导出文件格式保持兼容，更新后可继续使用现有数据与文件。
- Docker 自托管部署开箱即用：数据库、后端与前端一个命令启动，前端网关默认同源代理接口，长耗时生成与流式输出已适配，配套安装指南与常见问题文档。
- 自动导演运行详情的「N 项需复核」提示更准确：规划推进中被新书级方向替换的旧承诺不再计入需复核数量，正常演进不会以警告形式滞留在运行详情里。
- 自动导演的「缺少规划资源」提示只在书级约定、故事主线、角色阵容、分卷策略或章节任务单等规划资源确实缺失时出现；刚完成规划、尚未开始写正文时不再误报。
- 漫画导出文件与格子图、角色资产等本地图纸接口收紧了访问边界：越界的资源请求一律按不存在处理，不再可能读到应用数据之外的文件。
- 企业微信卡片里的"继续推进/换模型重试"链接在回调令牌未配置时会直接拒绝执行，不再可能出现被伪造链接误触任务操作的情况。
- 「仅不可批量」筛选现在能正确筛出不支持批量处理的跟进项，不再和「可批量」返回相同内容。
- 请求体格式错误（如非法 JSON）现在返回明确的参数错误提示，不再按服务器故障处理，也避免客户端误重试。
- 后台任务（生图队列、剧情副作用、向量索引）遇到数据库瞬时繁忙时会等待重试或记录日志，不再因此中断整个应用进程。
- 模型并发与调用间隔限制对流式生成同样生效，长章节写作时不再提前放行后续请求，减少上游限流报错。
- AI 实况面板的思考过程预览有长度上限，长思考模型的长时间生成不再占用持续增长的内存。
- 桌面端漫画面板、角色三视图、场景图与短剧剧照等图片现在能正常显示；漫画导出长图也能正常在浏览器中打开（此前在打包版中会打不开）。
- 漫画角色雪碧图的列宽按原图比例正确缩放，非正方形参考图不再出现拼图失败或空白条带。
- 榜单雷达中来源链接只允许跳转 http/https 网址；主题市场与短剧面板中的外部链接地址不合法时不再显示跳转入口。
- 从已有项目进入分卷规划时，AI 生成分卷或章节列表期间的手动编辑不再被轮询刷新冲掉。
- 全书状态重建更可靠：逐章更新状态，个别章节重建失败时会明确指出是哪几章，已完成章节的状态全部保留。
- 重新对已有小说发起接管或重跑大纲阶段时，早期章节保留的旧版章节合同（缺少章节目标、边界约定等字段）不再让整个任务直接失败：已完成正文不再被重复校验卡住，待执行章节的合同缺口会记录为待修复项，并在开始写该章前由 AI 自动补齐。
- 重新规划或重跑大纲后，角色动态会立即随最新规划重建，角色面板不再显示已被重置章节的过期时间线、阵营轨迹和关系阶段。
- 自动导演运行详情的步骤进度统计对长任务更准确，长时间运行后不再出现超出总数的进度显示。
- 漫画格子生成支持一次携带多张参考图（此前超过一张的部分会被忽略）。
- 关注中心与任务中心对已停滞的条目不再持续自动刷新，页面空闲时的后台请求显著减少。
- 章节审校现在会对照本书生效的写法规则逐条检查，问题清单直接指出违反了哪条规则并附正文依据，后续修文可以更准确地定向修复。
- 反AI写作检查补充了一批高频AI腔词表（模糊量词、条件反射式反应、氛围套话、表情公式等）；多章书籍的章节开头会附上「近几章这样开头」的规避样本，跨章复读开头被显式拦住；修文与改写候选要求在多种替换策略间轮换，不再全部收敛成「删解释、加小动作」一种模子。
- 章节编辑器新增范文锚点：采纳 AI 改写候选或一键标记选中段落即可收录为本书范文，后续章节写作会参考这些范文的句式与节奏，让文风更贴合作者本人；范文列表可随时查看和移除，未收录范文的新书会以拆书写法的源文本样本作初始参照。
- 章节计划会按叙事节奏为推进章安排代价、为转折章安排意外变量，并在写作和审校时检查它们是否被自然兑现，避免情节总是「顺利达标」的 AI 式顺滑感。
- 全书自动执行推进到最后一卷时不再因「当前卷节奏板的章节跨度异常」误报而暂停：收官卷会按既有规划继续拆章、写章直至完书；此时重新生成收官卷节奏板也不会再把整卷错误地压缩到只剩几章。
- 单章写作会自动守住篇幅上限：初稿一旦超过目标字数的硬性上限（约目标的 1.25 倍），会先自动压缩收束到可接受区间再保存，不用整章重写；审校阶段对仍超长的章节会给出明确的整章压缩修复项，个别模型把单章写到八千字以上也能收敛回正常篇幅，不会原样入库。
- 写法编辑页只说明当前窗口负责整理写法设定，应用测试和去 AI 味在独立入口；设置页在基础创作链路就绪时直接提示可以开始创建或继续推进小说。
...
- README 现在补充了对 [Ani Book Skill](https://github.com/weifu1997/ani-book-skill) 的独立介绍，方便想在 Codex 本地工作区里直接推进长篇中文小说的人，更快理解这条 Skill 化工作流和本项目完整工作台形态之间的关系。
