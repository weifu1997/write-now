...
- 自动导演运行详情的「N 项需复核」提示更准确：被新书级方向替换的旧承诺不再计入，正常规划推进不会滞留无法处理的警告。
- 自动导演的「缺少规划资源」提示只在规划资源确实缺失时出现；刚完成规划、尚未开始写正文时不再误报。
- 安全与稳定性修复：漫画图片与导出接口的本地文件访问收紧，企业微信回调链接在令牌未配置时拒绝执行，后台生图与索引任务遇到数据库瞬时繁忙不再中断应用；桌面端漫画/短剧图片与导出长图恢复正常显示；「仅不可批量」筛选生效；分卷生成期间的手动编辑不再被轮询冲掉。
- 稳定性继续增强：全书状态重建逐章进行并在失败时明确指出章节，长任务的导演进度统计更准确，漫画格子支持多张参考图，停滞页面不再常驻后台刷新。
- 章节审校会对照本书生效的写法规则逐条检查，问题直接指出违反了哪条规则并附正文依据，修文更有针对性。
- 反AI检查补充高频AI腔词表；章节开头附近几章开头规避样本；修文与改写候选在多种替换策略间轮换，避免单一模子。
- 章节编辑器新增范文锚点：采纳改写候选或一键标记段落即可收录为本书范文，后续写作参考其句式与节奏；范文可随时查看和移除。
- 重新对已有小说发起接管或重跑大纲时，早期章节保留的旧版章节合同不再让整个任务失败：已完成正文不被重复校验卡住，未写章节的合同缺口会在开写前由 AI 自动补齐。
- 重新规划或重跑大纲后，角色动态会立即随最新规划重建，角色面板不再显示已被重置章节的过期时间线、阵营轨迹和关系阶段。
