...
- 在章节编辑器里采纳改写结果或把段落存为范文时，不再因为正文超过固定 4000 字而提示参数校验失败。
- 章节篇幅仍按该章目标字数上下浮动来衡量；范文只保留一段代表性写法，整章保存不受影响。
- 简易书架的「一键自动修复全部质量项」会在同一次任务里先局部修补，仍不过或补丁打不上时自动改写，修完后再按当前正文重新审校；审校通过即关闭待跟进项，不用反复点按钮才能完成一轮升级。明显写坏的章节会直接尝试改写；两轮修复并重新审校后仍不过的局部问题会保留正文并继续显示待跟进。
- 全书自动创作在拆章完成后可以直接开写：章节任务单会在写每一章前即时补齐，不再要求先回到节奏/拆章把全书细化做完，也不会再被「缺少完整章节细化」误挡回大纲。
- 正文已经写出来后，局部审校缺口会记成待跟进质量项并继续推进，不会因此把整本自动导演任务直接判失败。
- 需要重规划时，暂停状态会明确标成重规划等待，恢复入口能走对分支，不再和普通批次暂停混在一起。
- 全书自动模式下，章节标题多样性问题默认记为提醒并继续，不再轻易整本停在大纲阶段；后台超时清理也不会悄悄清掉需要你确认的人工暂停。
